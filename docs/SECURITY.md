...
Review archives contain original source captures, identifiers and annotation text. Canonical checksums detect corruption, not authenticated provenance; restored notes remain permanently stale or absent and cannot supersede destination submissions. Task briefs likewise treat source and review text as data. Local MCP exposes eight read-only tools; it cannot scan, append reviews, import archives or execute lab adapters.

The opt-in [lifecycle lab](LIFECYCLE_LAB.md) runs trusted synchronous adapters for pinned public fixtures. Its source hashes and step bounds do not sandbox arbitrary callbacks or prove adapter completeness. The separate [pilot harness](PILOT_HARNESS.md) executes evaluator-configured commands and candidate code during protected tests; independent workspace copies and trace auditing do not establish hermetic read isolation. Those tools do not grant checker or policy authority in the assurance service.

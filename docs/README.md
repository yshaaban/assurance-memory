...
| [Successive maintenance decision](decisions/004-longitudinal-maintenance.md) | Understand automatic delivery, retained reasoning, review-subject gaps and the limits of the paired comparison |
...
| [1.4 validation and longitudinal results](VALIDATION_1_4.md) | Inspect all twelve assignments, integration failures, portability correction, effort and remaining evidence gaps |
...
| [Task-start workflow](TASK_WORKFLOW.md) | Deliver a refreshed brief, retain unverified prose and inspect source changes at the next task |
| [Longitudinal pilots](LONGITUDINAL_PILOTS.md) | Compare successive maintenance changes with separate histories and protected review gates |
...
Version 1.5.0 uses local schema 4 and adds candidate-free source observations, refined task retrieval and verified task-end archive export. Version 1.3 introduced task-oriented briefs, retained review archives, profiling and bounded evaluation tools. Local MCP exposes eight read-only tools; scanning, migration, review append and archive operations use the CLI. Restored annotations require local re-review. Preserve a SQLite-consistent backup for full source/drift/review/restore history. The assurance kernel remains the authority for reviewed requirements, evidence applicability, counterevidence and debt decisions. A brief, a lab observation or a current user note cannot establish complete application correctness. Earlier reports retain historical evidence; [1.4 validation](VALIDATION_1_4.md) retains the earlier implementation checks, frozen pilot outcomes and post-study corrections.

The current workflow adds [source observations](SOURCE_OBSERVATIONS.md), [task handoff](TASK_WORKFLOW.md), and [independent retrieval results](RETRIEVAL_1_5.md). See [1.5 validation](VALIDATION_1_5.md) for the integrated checks and fresh paired study.

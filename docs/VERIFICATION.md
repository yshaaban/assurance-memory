> Historical 1.0.0 authoring report. Some download-era `.log` files mentioned below are not included in Git. Commands, runtime requirements and counts below describe that original environment. For the current checked-in release, use [current validation](VALIDATION_1_1.md) and [getting started](GETTING_STARTED.md).

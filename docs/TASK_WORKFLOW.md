...
Packet output checks also compare file identity before SQLite opens, so case-normalized sidecar aliases and aliases reached through an existing database symlink fail cleanly. A dangling database leaf symlink is rejected before output reservation. Task-end archive names must remain distinct from DB/WAL/SHM names after Unicode normalization and case folding; this is intentionally conservative on case-sensitive filesystems.

...
import { searchTerms } from './local-search.js';
...
function taskTerms(task: string): { terms: string[]; ignored: number; omitted: number } {
  const normalized = searchTerms(task), terms: string[] = [];
  let ignored = 0, omitted = 0;
  for (const term of normalized) {
    if (stopWords.has(term)) { ignored++; continue; }
    if (terms.length >= 12 || Buffer.byteLength([...terms, term].join(' ')) > 384) { omitted++; continue; }
    terms.push(term);
  }
...
export function investigate(index: LocalIndex, task: string, count = 5, maxBytes = 24_000): Row {
  ...
  for (const hits of [...groups.values()].slice(0, count)) {
    ...
    if (encodedBytes() + count * 70 + 32 > maxBytes) {
      brief.entries.pop(); brief.omittedEntryIds.push(primary.id); brief.truncated = true;
    }
  }
